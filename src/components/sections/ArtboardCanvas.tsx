...
const COLUMN_SPEED_MULTIPLIERS = [1.0, 0.6, 1.0, 0.5, 1.0, 0.7, 1.0, 0.55, 1.0, 0.65];
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    useEffect(() => {
      ...
      const render = (time: number) => {
        ...
        const xOffset = offsetRef.current.x;
        const baseY = baseYOffsetRef.current;
        ...
        const gridSize = 100;
        const startX = (xOffset % gridSize) - gridSize; 
        const startY = (baseY % gridSize) - gridSize;
        ...
        itemRefs.current.forEach((el, id) => {
          ...
          const colIndex = ((col % COLUMN_SPEED_MULTIPLIERS.length) + COLUMN_SPEED_MULTIPLIERS.length) % COLUMN_SPEED_MULTIPLIERS.length;
          const speedMultiplier = COLUMN_SPEED_MULTIPLIERS[colIndex];
          
          const itemX = centerX + col * cellW + xOffset;
          const itemY = centerY + row * cellH + baseY * speedMultiplier;
          
          el.style.transform = `translate3d(${itemX}px, ${itemY}px, 0)`;
        });

        const buffer = 1;
        const newMinC = Math.floor((-xOffset - centerX) / cellW) - buffer;
        const newMaxC = Math.ceil((window.innerWidth - xOffset - centerX) / cellW) + buffer;
        const newMinR = Math.floor((-baseY - centerY) / cellH) - buffer;
        const newMaxR = Math.ceil((window.innerHeight - baseY - centerY) / cellH) + buffer;
